...
--// === Hitbox Size Adjustment Section ===
local function makeSlider(text, min, max, default, callback)
	...
	return withPadding(sliderFrame)
end

--// Hitbox Adjustment
local function updateHitbox(name, value)
	if character then
...
-- Add Hitbox sliders
local hitboxSliders = {
	makeSectionTitle("Hitbox Size Adjustments"),
	makeSlider("BlockHitBox", 1, 10, 2, function(v) updateHitbox("BlockHitBox", v) end),
	makeSlider("DiveHitBox", 1, 10, 2, function(v) updateHitbox("DiveHitBox", v) end),
	makeSlider("RecHitBox", 1, 10, 2, function(v) updateHitbox("RecHitBox", v) end),
	makeSlider("TopHitBox", 1, 10, 2, function(v) updateHitbox("TopHitBox", v) end),
}

for _, el in ipairs(hitboxSliders) do
	el.Parent = frame
end

-- Optional: automatically apply size when character respawns
player.CharacterAdded:Connect(function()
	task.wait(0.5)
	for _, el in ipairs(hitboxSliders) do
		if el:IsA("Frame") and el:FindFirstChildWhichIsA("TextLabel") then
			local text = el:FindFirstChildWhichIsA("TextLabel").Text
			local name = text:match("^(%w+):")
			local value = tonumber(text:match(": ([%d%.]+)"))
			if name and value then
				updateHitbox(name, value)
			end
		end
	end
end)

print("[J Hub Hub] Hitbox adjustment sliders loaded.")
